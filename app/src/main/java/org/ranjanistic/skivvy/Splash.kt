...
//        window.setBackgroundDrawable(getDrawable(R.drawable.ic_yellow_dotsincircle))
                    ...
                    when(data.getStringArrayListExtra(RecognizerIntent.EXTRA_RESULTS)!![0].toString()
                        ...
                        "fingerprint"->{
                            if(skivvy.getBiometricStatus()) {
                                ...
                                if(temp.getAuthAttemptCount()>0)
                                    speakOut(getString(R.string.biometric_is_off), skivvy.CODE_VOICE_AUTH_CONFIRM)
                                else
                            ...
                            if (temp.getAuthAttemptCount() > 0) {
                                if(temp.getAuthAttemptCount() == 1){
                                    speakOut("Last chance", skivvy.CODE_VOICE_AUTH_CONFIRM)
                                } else {
                                    speakOut("${temp.getAuthAttemptCount()} chances left", skivvy.CODE_VOICE_AUTH_CONFIRM)
                                }
                            } else {
                                if(skivvy.getBiometricStatus()) {
                                    ...
                                    speakOut("Authentication failed.")
                                    finish()
                                }
                            }
                        }
    ...
    private fun startVoiceRecIntent(code: Int, msg:String) {
        val intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH)
            ...
            intent.resolveActivity(packageManager)?.let{startActivityForResult(intent, code)}
    }
